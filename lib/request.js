...
class Request {
    ...
    send(value) {
        ...
        return this._promise.then(() => {
            ...
            this.log(`[HTTP Request] [Method=${this._method}] [URL=${this._url}] HEADER=${_header} BODY=${_body}`);
            ...
            return new Promise((resolve, reject) => {
                try {
                    ...
                    if (this._circuitBreaker) {
                        promise = this._circuitBreaker.check(this.log);
                    }
                    ...
                    promise.then(() => {
                            ...
                            try {
                                ...
                                res.on("end", () => {
                                    ...
                                    if (statusCode < 400)
                                        ...
                                        if (this._circuitBreaker) {
                                            this._circuitBreaker.breakOnError(this.log, statusCode, headers);
                                        }
                                        reject(this._result);
                                    }
                                });
                        ...
                        req.on("error", err => {
                            console.log("Send HTTP request failed", err);
                            reject(err);
                        })
    ...
    logger(value, encoding="utf8", logHeader = false, logBody = true) {
        this.log = value;
    ...
    circuitBreaker(name) {
        if (name) {
            this._circuitBreaker = circuitBreaker(name);
        }
        return this;
    }
}
